...
router.get('/commit',verifyToken,requireRole('ผู้รับการประเมินผล'),async (req,res) => {
    try{
        ...
        const [rows] = await db.query(`select * from tb_member m,tb_eva e,tb_system s,tb_commit c where e.id_member=? and e.id_eva=c.id_eva and c.id_member=m.id_member and e.id_sys=s.id_sys `,[id_member])
        res.json(rows)
    }catch(err){
...
router.get('/scores',verifyToken,requireRole('ผู้รับการประเมินผล'),async (req,res) => {
    try{
        ...
        rows.forEach(row =>{
            ...
            if(row.status_eva === 2) scores[row.id_indicate].a = row.score_member*row.point_indicate
            if(row.status_eva === 3) scores[row.id_indicate].a = row.score_member*row.point_indicate
            if(row.status_eva === 4) scores[row.id_indicate].a = row.score_member*row.point_indicate
            totalScore += (row.score_member*row.point_indicate)/3
        })
